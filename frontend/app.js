...
window.addEventListener('hashchange', () => {
    ...
    if (['dashboard', 'repos', 'settings'].includes(page)) {
...
function resetRepoForm() {
    ...
    document.getElementById('repoModalTitle').textContent = 'Add Repository Pair';
...
function editRepoPair(id) {
    ...
    document.getElementById('repoModalTitle').textContent = 'Edit Repository Pair';
    repoModal.show();
}
        ...
        container.innerHTML = logs.map(log => `
            <div class="log-entry">
                ...
                <div class="small">
                    ...
                    ${log.branches_synced ? `<br><span class="text-muted">Branches: ${log.branches_synced.length}</span>` : ''}
                    ${log.tags_synced !== undefined ? `<span class="text-muted ms-2">Tags: ${log.tags_synced}</span>` : ''}
                </div>
            </div>
        `).join('');
    } catch (error) {
...
// Initial load - use URL hash to determine starting page
initFromHash();
